document.addEventListener("DOMContentLoaded", () => {
    function checkScreenWidth() {
        if (window.innerWidth < 962) {
            const animateCssLink = document.getElementById("animateCss");
            if (animateCssLink) {
                animateCssLink.parentNode.removeChild(animateCssLink);
            }
        }
    }

    checkScreenWidth(); // Check on page load

    window.addEventListener('resize', () => {
        checkScreenWidth(); // Check on window resize
    });
});
